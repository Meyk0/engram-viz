...
| `engram dev [--port 3100] [--executor module]` | Start Studio and optionally attach a real replay executor |
...
Attach a project-owned replay executor when incidents should rerun the actual
agent pipeline instead of using the built-in context-only fallback:
...
npx --yes @engramviz/cli run -- node scripts/reproduce-incident.mjs
```
